...
class BEGAN128(BaseModel):
    def __init__(self, name, training, D_lr=1e-4, G_lr=1e-4, image_shape=[128, 128, 1], z_dim=64, gamma=0.5):
        self.gamma = gamma
        ...
        super(BEGAN128, self).__init__(name=name, training=training, D_lr=D_lr, G_lr=G_lr, 
    ...
    def _encoder(self, X, reuse=False):
        with tf.variable_scope('encoder', reuse=reuse):
            ...
            with slim.arg_scope([slim.conv2d], kernel_size=[3,3], padding='SAME', activation_fn=tf.nn.elu):
                ...
                net = slim.conv2d(net, nf)
                net = slim.conv2d(net, nf*2, stride=2) # 64x64

                net = slim.conv2d(net, nf*2)
                net = slim.conv2d(net, nf*2)
                net = slim.conv2d(net, nf*3, stride=2) # 32x32

                net = slim.conv2d(net, nf*3)
                net = slim.conv2d(net, nf*3)
                net = slim.conv2d(net, nf*4, stride=2) # 16x16

                net = slim.conv2d(net, nf*4)
                net = slim.conv2d(net, nf*4)
                net = slim.conv2d(net, nf*5, stride=2) # 8x8

                net = slim.conv2d(net, nf*5)
                net = slim.conv2d(net, nf*5)
                net = slim.conv2d(net, nf*5)
