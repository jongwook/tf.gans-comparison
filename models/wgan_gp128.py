...
class WGAN_GP128(BaseModel):
    def __init__(self, name, training, D_lr=1e-4, G_lr=1e-4, image_shape=[128, 128, 1], z_dim=100):
        self.beta1 = 0.0
        self.beta2 = 0.9
        self.ld = 10. # lambda
        self.n_critic = 5
        super(WGAN_GP128, self).__init__(name=name, training=training, D_lr=D_lr, G_lr=G_lr,
    ...
    def _good_generator(self, z, reuse=False):
        with tf.variable_scope('generator', reuse=reuse):
            ...
            net = self._residual_block(net, nf // 2, resample='up', name='res_block5') # 128x128x32
            expected_shape(net, [128, 128, 32])
            net = slim.batch_norm(net, activation_fn=tf.nn.relu, **self.bn_params)
            net = slim.conv2d(net, 1, kernel_size=[3,3], activation_fn=tf.nn.tanh)
            expected_shape(net, [128, 128, 1])
    ...
    def _good_critic(self, X, reuse=False):
        with tf.variable_scope('critic', reuse=reuse):
            ...
            net = self._residual_block(net, nf, resample='down', name='res_block1') # 128x128x64
            net = self._residual_block(net, 2*nf, resample='down', name='res_block2') # 32x32x128
            net = self._residual_block(net, 4*nf, resample='down', name='res_block3') # 16x16x256
            net = self._residual_block(net, 8*nf, resample='down', name='res_block4') # 8x8x512
            net = self._residual_block(net, 8*nf, resample='down', name='res_block5') # 4x4x512
            expected_shape(net, [4, 4, 512])
            net = slim.flatten(net)
            net = slim.fully_connected(net, 1, activation_fn=None)
